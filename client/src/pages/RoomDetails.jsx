...
const RoomDetails = () => {
  ...
  return room &&(
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        {/* Room Details  */}
        <div className='flex flex-col md:flex-row items-start mid-items-center gap-2'>
            <h1 className='text-3xl md:text-4xl font-playfair '>
                {room.hotel.name} <span className='font-intert text-sm'>({room.roomType})</span></h1>
        ...
        <div className='flex flex-col lg:flex-row mt-6 gap-6'>
            ...
            <div className='grid grid-cols-2 lg:w-1/2 w-full gap-4'>
                {room.images.length > 1 && room.images.map((image, index) => (
                    ...
                    key={index} src={image} alt="RoomImage" className={`w-full rounded-xl shadow-md object-cover cursor-pointer ${mainImage === image && 'outline-3 outline-orange-500'}`} />
                ))}
            </div>
        </div>
        ...
        <div className='flex flex-col items-start gap-4'>
            <div className='flex gap-4'>
                <img src="https://media.istockphoto.com/id/1452858525/vector/letter-m-minimal-icon-design-template-elements.jpg?s=1024x1024&w=is&k=20&c=ahBDTNDZIeS5arwR1Gu1mMOCHUNEzijfWLpjmBEGwwY=" alt="Host" 
